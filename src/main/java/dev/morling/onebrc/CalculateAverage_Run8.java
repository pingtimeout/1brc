...
public class CalculateAverage_Run8 {
    ...
    private static class Task implements Runnable {
        ...
        private Map<Key, Measurement> measurements;

        private Task(int ordinal, long chunk, long length, RandomAccessFile raf, CountDownLatch doneSignal) {
        ...
        @Override
        public void run() {
            measurements = extractMeasurementsFromChunk(ordinal, chunk, length, raf);
            doneSignal.countDown();
        }

        public Map<Key, Measurement> getMeasurements() {
            return measurements;
        }
    }
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        final long chunk = length / numCpu;
        final RandomAccessFile raf = new RandomAccessFile(file, "r");
        final CountDownLatch countDownLatch = new CountDownLatch(numCpu);
        final AtomicReference<Map<Key, Measurement>> result = new AtomicReference<>(new HashMap<>());
        final List<Task> tasks = new ArrayList<>(numCpu);

        for (int i = 0; i < numCpu; i++) {
            Task task = new Task(i, chunk, length, raf, countDownLatch);
            tasks.add(task);
        ...
        Map<Key, Measurement> allMeasurementsMap = tasks.stream()
                .map(Task::getMeasurements)
                .reduce(CalculateAverage_Run8::mergeMeasurementMaps)
